...
std::unique_ptr<LogicExpressionNode> LogicExpression::build(ScAddr node) {
    ...
    if (atomicFormula->Next()) {
        ...
        SC_LOG_INFO(context->HelperGetSystemIdtf(node) + " is a template")
        ...
        SC_LOG_INFO(context->HelperGetSystemIdtf(node) + " is a conjunction")
        if (operands->Next()) {
        ...
        SC_LOG_INFO(context->HelperGetSystemIdtf(node) + " is a disjunction")
        if (operands->Next()) {
            std::unique_ptr<LogicExpressionNode> opA =
                    build(operands->Get(2));
        ...
        SC_LOG_INFO(context->HelperGetSystemIdtf(node) + " is a not")
        if (operands->Next()) {
